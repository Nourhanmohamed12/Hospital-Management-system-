...
class Ui_Table(object):
    def setupUi(self, Table):
        ...
        curs = hosp.cursor()
        curs.execute('Select count(*) from Doctors')
        row = curs.fetchone()
        counts = row[0]
        icon = QtGui.QIcon()
        ...
        self.tableWidget.setHorizontalHeaderLabels(['DoctorID', 'FirstName', 'LastName', 'DateOfBirth', 'Gender','PhoneNumber','Specialty','receptionID'])
